...
\subsubsection{The field of intervals}
...
\noindent These operations are trivially commutative and associative. Additionally, let $\alpha, \beta, \gamma \in \mathcal{A}_0$, then $(\alpha + \beta) \gamma = \alpha \gamma + \beta \gamma$.
...
&= \{ac; a \in \alpha, c \in \gamma\} + \{bc ; b \in \beta, c \in \gamma\} \quad \text{(by definition of the sum)} \\
&= \alpha \gamma + \beta \gamma
\end{align*}
\hfill{} $\boxed{}$

This gives us a ring structure for $(\mathcal{A}_0, +, \cdot)$, which we will use just next, and form now on we will note $0 = [0, 0]$ and $1 = [1, 1]$. One important thing is that $\mathcal{A}_0$ is a field, i.e.:
...
\subsubsection{Centered intervals}
...
\begin{enumerate}[i]
    ...
    \item The case when $\alpha$ is a real interval is easy. Using property \ref{muscalarprod} and the ring structure of $\mathcal{A}_0$, the general case is proven.
    \hfill{} $\boxed{}$
\end{enumerate}

\noindent From these properties, it follows that $\mu$ is a ring morphism.
...
Now let $\alpha^c$ be this centered interval $\alpha - \mu(\alpha)$, and $\mathcal{A}_0^c = \text{Ker}(\mu)$ be the ring of centered intervals. What makes this decomposition particularly interesting is that $\pm : \mathbb{C} \rightarrow \mathcal{A}_0^c$ is a ring morphism.
...
\subsubsection{Partial order on intervals}
