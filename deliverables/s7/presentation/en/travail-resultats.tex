...
\begin{frame}[noframenumbering]{Theoretical model} % with colors
...
\begin{frame}[noframenumbering]{Theoretical model} % with colors and schema
...
\begin{frame}[noframenumbering]{Theoretical model} % with colors and schema and faith
...
\begin{frame}{Theoretical model}
  Example : we get the additive diagram
  \begin{center}
  ...
  \pause
  Let's \textbf{reduce} this diagram
\end{frame}

\begin{frame}{Theoretical model}
  We can force the merge of $A$ and $B$
  % https://tikzcd.yichuanshen.de/#
  \begin{tikzcd}[ampersand replacement=\&, column sep=huge, row sep=large]
    \& D \arrow[rd, "i"] \arrow[ld, "4i"', dashed] \arrow[rd, dashed, bend right=49] \&                                                     \\
  \color{blue}{A} \arrow[rd, "\frac52"', dashed, bend right=49, blue] \arrow[rd, blue] \&                                                                               \& \color{cs}{B} \arrow[ld, "2"', dashed, cs] \arrow[ld, bend left=49, cs] \\
...
\begin{frame}{Results}
  We can always reduce diagrams more
  $$\Downarrow$$
  Arbitrarily large space gain (up to exponential)
\end{frame}

\begin{frame}{Error}
  How to choose which diagrams to merge?
  $$\Downarrow$$
  \textbf{Error} : we can compute the error induced by a diagram
\end{frame}

\begin{frame}[noframenumbering]{Gate-basics}
...
\begin{frame}[noframenumbering]{During gate 1}
...
\begin{frame}[noframenumbering]{After gate}
